    ...
    char **argv)
{
    ...
    try
    {
        ...
        Client<ProtoStrings> client(addr, port);
        std::string requestString;
        while (std::cout << "request 'string will be sent as 0-terminated'> ", std::cin >> requestString)
        {
            RequestData requestBuffer(requestString.begin(), requestString.end());
            requestBuffer.push_back(0);
            AnswerData answerBuffer = client.request(requestBuffer);
