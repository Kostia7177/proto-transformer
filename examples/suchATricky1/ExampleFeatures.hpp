...
struct SetAnswerSize
{
    void operator()(uint32_t size, AnyAnswerHdr &answerHdr) { *(uint32_t *)(answerHdr.itself + sizeof(uint32_t)) = htonl(size); }
};

struct GetAnswerSize
{
    uint32_t operator()(const AnyAnswerHdr &answerHdr) { return ntohl(*(uint32_t *)(answerHdr.itself + sizeof(uint32_t))); }
};
